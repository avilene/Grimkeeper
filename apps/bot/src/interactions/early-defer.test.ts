...
import { shouldDeferStReminderCommand } from "./early-defer.js";
...
describe("shouldDeferStReminderCommand", () => {
  it("defers reminder subcommands in full mode", () => {
    ...
    expect(shouldDeferStReminderCommand(chatCommand("st", "reminders") as never)).toBe(true);
    expect(shouldDeferStReminderCommand(chatCommand("st", "remind") as never)).toBe(true);
    expect(shouldDeferStReminderCommand(chatCommand("st", "set-reminders") as never)).toBe(true);
  });

  it("skips non-reminder /st subcommands in full mode", () => {
    vi.mocked(isMinimalMode).mockReturnValue(false);
    expect(shouldDeferStReminderCommand(chatCommand("st", "execute") as never)).toBe(false);
  });

  it("defers all /st commands in minimal mode", () => {
    vi.mocked(isMinimalMode).mockReturnValue(true);
    expect(shouldDeferStReminderCommand(chatCommand("st", "reminders") as never)).toBe(true);
    expect(shouldDeferStReminderCommand(chatCommand("st", "execute") as never)).toBe(true);
  });

  it("skips non-/st commands", () => {
    vi.mocked(isMinimalMode).mockReturnValue(false);
    expect(shouldDeferStReminderCommand(chatCommand("game", "create") as never)).toBe(false);
  });
});
